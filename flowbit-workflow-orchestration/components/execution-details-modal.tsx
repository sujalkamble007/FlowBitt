...
export function ExecutionDetailsModal({ open, onOpenChange, executionId, engine }: ExecutionDetailsModalProps) {
  ...
  const { logs: sseLogs, connected: sseConnected } = useSSELogs(
    engine === "langflow" ? executionId : null
  )
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-6xl max-h-[90vh]">
        ...
        <Tabs defaultValue="overview" className="w-full">
          ...
          <TabsContent value="message-logs" className="space-y-4">
            <ScrollArea className="h-96">
              {sseLogs.length > 0 ? (
                <div className="space-y-2">
                  {sseLogs.map((log, idx) => (
                    <div key={idx} className="text-xs font-mono bg-black text-green-400 p-1 rounded">
                      {log}
                    </div>
                  ))}
                  {!sseConnected && (
                ...
                <div className="text-center py-8 text-gray-500">
                  <Info className="w-8 h-8 mx-auto mb-2" />
                  <p>No real-time logs available for this execution</p>
                </div>
              )}
            </ScrollArea>
          </TabsContent>
